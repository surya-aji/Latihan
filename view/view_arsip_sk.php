...
if(isset($_GET['skid'])){
			...
			<tbody><?php
				...
				foreach($dump_sk as $key => $object){
					...
					<tr>
						...
						<td><?php echo sprintf("%04d", $object->id_sk);?></td>
						<td><a href="./index.php?op=sk&skid=<?php echo $object->id_sk;?>"><?php echo $object->no_sk;?></a></td>
	...
	if($pg > 1){
		$link = $pg-1;
		$prev = "index.php?op=arsip_sk&halaman=$link$link_order";
		$prev_disable = " ";
	}else{
		$prev = "#";
	...
	if($pg < $JmlHalaman){
		$link = $pg + 1;
		$next = "index.php?op=arsip_sk&halaman=$link$link_order";
		$next_disable = " ";
	}else{
		$next = "#";
